...
def render_profile_card(row, search_term):
    ...
    b64_img = encode_image_to_base64(os.path.join("images", row['Image']))
    image_html = (
    ...
    # Highlighted fields
    highlighted_name = highlight_match(row['Fullname'], search_term)
    highlighted_email = highlight_match(row['Account'], search_term)

    # Format AI Tools
    tools_html_lines = []
    for line in str(row['AI Tools']).split('\n'):
        if ':' in line:
